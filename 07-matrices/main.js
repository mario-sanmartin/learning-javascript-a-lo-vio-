...
// const perros = listado.map(elemento => elemento.textContect);
...
const perros = Array.from(
    document.querySelectorAll("li"),
    elemento => elemento.textContent
  );

  const mascotas = perros.forEach(elemento =>{
      console.log(elemento);
  })

  function total (n1,n2,n3,n4){
    //   console.log(arguments);
    return Array.from(arguments).reduce((ant,sig) => ant + sig , 0);
  }

console.log(total(2,77,55,77));
...
//Array Find
/* Devuelve el primero que cumpla con la funcion que pasaremos sobre
cada uno de los elementos de la matriz */
const corroe = 'umut.velioglu@example.com';
const usuarioSeleccionado = users.find(user => user.email === corroe).name.last;


//Array findIndex
/**
Busca el indice requerido 
*/
const uuid= 'f1d62a3a-687b-4554-af54-933108e357e6';
const buscadin = users.findIndex(user => user.login.uuid === uuid);

//Splice para borrar el elemento
/* splice retorna lo que borra, ahora
le pedimos que entre en buscadin y borre 1*/
// users.splice(buscadin,1);
/*
Y si ahora buscamos Users nos faltara el elemento
con id 6
*/

//Array Filter
/*
Crea una nueva matriz/array con todos los elementos
que cumplan la condición/ que pasen el test
en este caso que sean de sexo femenino
*/
const usuariosHembras = users.filter(user =>(user.gender == 'female'));


//Array.of + validacion con includes
console.log(Array.of(1,2,3,4,5,6).includes(1));
//el array incluye el 1 ? true.

console.log(Array.of(1,2,3,4,5,6).includes(7));
//el array incluye el 7 ? false , esta o no esta basicamente.